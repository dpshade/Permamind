...
describe("Cross-Hub Discovery MCP Tools", () => {
  ...
  beforeAll(async () => {
    ...
    const { CrossHubDiscoveryService } = await import(
      "../dist/services/CrossHubDiscoveryService.js"
    );
    const { WorkflowPerformanceTracker } = await import(
      "../dist/services/WorkflowPerformanceTracker.js"
    );
    const { WorkflowRelationshipManager } = await import(
      "../dist/services/WorkflowRelationshipManager.js"
    );
    const { WorkflowEnhancementEngine } = await import(
      "../dist/services/WorkflowEnhancementEngine.js"
    );
    const { WorkflowAnalyticsService } = await import(
  ...
  describe("discoverCrossHubWorkflows Tool", () => {
    ...
    it("should validate required parameters for capability discovery", async () => {
      ...
      expect(result).toBe(
        "capability parameter required for capability discovery",
      );
    });
    ...
    it("should validate required parameters for requirements discovery", async () => {
      ...
      expect(result).toBe(
        "requirements parameter required for requirements discovery",
      );
    });
    ...
    it("should validate required parameters for similarity discovery", async () => {
      ...
      expect(result).toBe(
        "localWorkflowId parameter required for similarity discovery",
      );
    });
    ...
    it("should reject invalid discovery types", async () => {
      ...
      expect(result).toBe(
        "Invalid discoveryType. Use: capability, requirements, similar, or search",
      );
    });

    it("should handle capability discovery with mock data", async () => {
      // Mock the discovery service method
      workflowServices.crossHubDiscovery.discoverByCapability = vi
        .fn()
        .mockResolvedValue([
          {
    ...
    it("should handle requirements discovery with parsing", async () => {
      workflowServices.crossHubDiscovery.findWorkflowsForRequirements = vi
        .fn()
        .mockResolvedValue([
          {
            workflowId: "bulk-processor",
      ...
      expect(
        workflowServices.crossHubDiscovery.findWorkflowsForRequirements,
      ).toHaveBeenCalledWith(["large-datasets", "real-time"]);
    });

    it("should handle search with filter parsing", async () => {
      workflowServices.crossHubDiscovery.searchGlobalWorkflows = vi
        .fn()
        .mockResolvedValue([]);
      ...
      expect(
        workflowServices.crossHubDiscovery.searchGlobalWorkflows,
      ).toHaveBeenCalledWith("financial reporting", {
        capabilities: ["analysis", "reporting"],
        tags: ["finance", "public"],
        minReputationScore: 0.8,
        minPerformanceScore: 0.9,
    ...
    it("should limit results correctly", async () => {
      ...
      workflowServices.crossHubDiscovery.discoverByCapability = vi
        .fn()
        .mockResolvedValue(manyWorkflows);
  ...
  describe("getNetworkStatistics Tool", () => {
    ...
    it("should return network statistics", async () => {
      workflowServices.crossHubDiscovery.getNetworkStatistics = vi
        .fn()
        .mockResolvedValue({
          totalHubs: 5,
  ...
  describe("requestEnhancementPatterns Tool", () => {
    ...
    it("should request enhancement patterns from specified workflow", async () => {
      ...
      workflowServices.crossHubDiscovery.requestEnhancementPatterns = vi
        .fn()
  ...
  describe("discoverHubs Tool", () => {
    ...
    it("should discover and return hub information", async () => {
      ...
      workflowServices.crossHubDiscovery.discoverHubs = vi
        .fn()
        .mockResolvedValue(mockHubs);
    ...
    it("should handle force refresh parameter", async () => {
      ...
      expect(
        workflowServices.crossHubDiscovery.discoverHubs,
    ...
    it("should handle service method errors gracefully", async () => {
      ...
      const discoverTool = {
        execute: async (args) => {
          try {
            const workflows =
              await workflowServices.crossHubDiscovery.discoverByCapability(
                args.capability,
              );
