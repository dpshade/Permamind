...
describe('Cross-Hub Discovery Tests', () => {
  ...
  describe('Event to Workflow Conversion', () => {
    it('should convert event to CrossHubWorkflow format', () => {
      ...
      const workflow = discoveryService.convertEventToCrossHubWorkflow(mockEvent, 'test-hub-id');
    ...
    it('should handle events with missing optional fields', () => {
      ...
      const workflow = discoveryService.convertEventToCrossHubWorkflow(minimalEvent, 'minimal-hub');
    ...
    it('should handle malformed performance data gracefully', () => {
      ...
      expect(() => {
        const workflow = discoveryService.convertEventToCrossHubWorkflow(eventWithBadPerformance, 'test-hub');
  ...
  describe('Reputation Scoring', () => {
    it('should calculate reputation score for events with full data', () => {
      ...
      const score = discoveryService.calculateEventReputationScore(fullEvent, performanceMetrics);
    ...
    it('should handle events with minimal data', () => {
      ...
      const score = discoveryService.calculateEventReputationScore(minimalEvent, minimalMetrics);
    ...
    it('should weight performance and reliability higher', () => {
      ...
      const highScore = discoveryService.calculateEventReputationScore(highPerfEvent, highPerfMetrics);
      const lowScore = discoveryService.calculateEventReputationScore(lowPerfEvent, lowPerfMetrics);
  ...
  describe('Filter Matching', () => {
    it('should match workflows against additional filters', () => {
      ...
      expect(discoveryService.matchesAdditionalFilters(highQualityWorkflow, strictFilters)).toBe(true);
      expect(discoveryService.matchesAdditionalFilters(lowQualityWorkflow, strictFilters)).toBe(false);
    });
    ...
    it('should pass when no filters are specified', () => {
      ...
      expect(discoveryService.matchesAdditionalFilters(anyWorkflow, {})).toBe(true);
    });
  });
  ...
  describe('Similarity Filtering', () => {
    it('should filter workflows by capability overlap', () => {
      ...
      const similar = discoveryService.filterBySimilarity(workflows, localWorkflow);

      expect(similar).toHaveLength(2);
      expect(similar.map(w => w.workflowId)).toEqual(['similar-1', 'similar-2']);
    });
  ...
  describe('Query Matching', () => {
    it('should match workflows against search queries', () => {
      ...
      expect(discoveryService.matchesQuery(workflow, 'data analysis')).toBe(true);
      expect(discoveryService.matchesQuery(workflow, 'reporting automation')).toBe(true);
      expect(discoveryService.matchesQuery(workflow, 'machine learning')).toBe(false);
    });
  ...
  describe('Network Statistics', () => {
    it('should generate network statistics with mock data', async () => {
      ...
      discoveryService.queryHubWorkflows = vi.fn()
        .mockResolvedValueOnce([
          { capabilities: ['data-analysis', 'reporting'], reputationScore: 0.8 },
          { capabilities: ['data-analysis', 'visualization'], reputationScore: 0.9 }
        ])
        .mockResolvedValueOnce([
  ...
  describe('Enhancement Pattern Processing', () => {
    it('should parse enhancement patterns from events', () => {
      const mockEnhancementEvent = {
        ...
        workflow_enhancement: '{"type":"optimization","impact":0.35,"description":"Parallel data chunking"}',
      ...
      const extractedPattern = {
        ...
        description: JSON.parse(mockEnhancementEvent.workflow_enhancement).description,
        impact: parseFloat(mockEnhancementEvent.enhancement_impact),
        applicableToCapabilities: Array.isArray(mockEnhancementEvent.workflow_capability) 
      ...
      expect(extractedPattern.applicableToCapabilities).toEqual(['data-processing', 'analytics']);
  ...
  describe('Error Handling', () => {
    it('should handle network errors gracefully', async () => {
      ...
      const patterns = await discoveryService.requestEnhancementPatterns('failing-hub', 'test-workflow');
  ...
  describe('Caching Behavior', () => {
    it('should demonstrate caching concept', () => {
      ...
      const mockResult = [{ workflowId: 'cached-workflow', reputationScore: 0.8 }];
