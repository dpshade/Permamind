...
const patterns = await fetchEvents(sourceHubId, JSON.stringify([enhancementFilter]));
```
...
const publicOnly = events.filter(e => 
  e.ai_tag?.includes("public") || e.ai_tag?.includes("discoverable")
);
...
const similarWorkflows = await discoveryService.discoverByCapability("data-processing");
...
const betterPerformers = similarWorkflows.filter(w => 
...
const compatiblePatterns = patterns.filter(p => 
  p.applicableToCapabilities.some(cap => myWorkflow.capabilities.includes(cap)) &&
  p.riskLevel === "low"
);
