...
describe("HubRegistryService", () => {
  ...
  describe("create", () => {
    it("should create new hub with profile", async () => {
      const { createProcess, createProfile } = await import("../../../src/process.js");
      ...
      expect(createProfile).toHaveBeenCalledWith(mockKeyPair, mockProcessId, mockProfileData);
    ...
    it("should handle createProcess failure", async () => {
      ...
      vi.mocked(createProcess).mockRejectedValue(new Error("Process creation failed"));

      await expect(service.create(mockKeyPair, mockProfileData))
    ...
    it("should handle evaluateHub failure", async () => {
      ...
      vi.mocked(evaluateHub).mockRejectedValue(new Error("Hub evaluation failed"));
  ...
  describe("getZoneById", () => {
    ...
    it("should handle getZone failure", async () => {
      ...
      await expect(service.getZoneById("invalid-process", "invalid-owner"))
  ...
  describe("register", () => {
    ...
    it("should handle registration failure", async () => {
      ...
      vi.spyOn(service, 'register').mockRejectedValue(new Error("Registration failed"));
  ...
  describe("profile data validation", () => {
    it("should handle minimal profile data", async () => {
      ...
      expect(createProfile).toHaveBeenCalledWith(mockKeyPair, "test-process", minimalProfile);
    });
    ...
    it("should handle complete profile data", async () => {
      ...
      expect(createProfile).toHaveBeenCalledWith(mockKeyPair, "complete-process", completeProfile);
    });
  });
});