...
describe("Token Tools Integration", () => {
  ...
  it("should integrate token tools with registry", () => {
    ...
    const saveTokenDefinition = toolDefinitions.find(def => def.name === "saveTokenMapping");
  ...
  it("should execute saveTokenMapping tool successfully", async () => {
    ...
    expect(parsedResult.mapping.processId).toBe("abcdefghijklmnopqrstuvwxyz1234567890abcdefg");
    ...
    expect(event).toHaveBeenCalledWith(
      ...
      expect.arrayContaining([
        ...
        expect.objectContaining({ name: "token_processId", value: "abcdefghijklmnopqrstuvwxyz1234567890abcdefg" }),
      ])
    );
  });
  ...
  it("should execute listTokens tool successfully", async () => {
    ...
    const mockEvents = [
      { token_name: "Test Token", token_ticker: "TEST", token_processId: "process1" },
      { token_name: "Another Token", token_ticker: "OTHER", token_processId: "process2" },
    ];
  ...
  it("should execute getTokenBalance tool successfully", async () => {
    ...
    (fetchEvents as any).mockResolvedValue([
      { token_name: "Test Token", token_ticker: "TEST", token_processId: "abcdefghijklmnopqrstuvwxyz1234567890abcdefg" }
    ]);
    
    // Mock balance query
    const mockBalance = { Data: JSON.stringify({ balance: "1000" }) };
    (read as any).mockResolvedValue(mockBalance);
