...
export interface DefaultProcessService {
   ...
   */
  getDefaultProcess(type: string, processId?: string): ProcessDefinition | null;
  ...
  ): ProcessTypeDetection | null;

  /**
   * Check if a process ID matches any known default patterns
   */
  isKnownProcessType(processId: string, handlers?: string[]): string | null;
    ...
    detectProcessType(
      ...
      if (isTokenProcess(handlers)) {
        ...
        const confidence = Math.min(
          (tokenHandlers.length / TOKEN_DETECTION_PATTERNS.handlers.length) * 0.8 + 0.2,
          1.0
        );
