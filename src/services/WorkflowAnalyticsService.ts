...
export class WorkflowAnalyticsService {
  ...
  generateRecommendations(workflowId?: string): string[] {
    ...
    if (
      ...
      if (lowPerformers.length > 0) {
        recommendations.push(
          `Focus optimization efforts on ${lowPerformers.join(", ")} workflows`,
        );
      }
    }
    ...
    if (enhancement.successRate < 0.6) {
      recommendations.push(
        "Improve enhancement validation process - current success rate is below 60%",
      );
    }

    if (enhancement.averageImpact < 0.3) {
      recommendations.push(
        "Target higher-impact enhancements - current average impact is low",
      );
    }
    ...
    if (collaboration.networkDensity < 0.3) {
      recommendations.push(
        "Increase workflow collaboration - network density is low",
      );
    }
    ...
    if (learning.learningRate < 1) {
      recommendations.push(
        "Increase learning frequency - currently less than 1 improvement per day",
      );
    }
  ...
  getCollaborationMetrics(): CollaborationMetrics {
    ...
    const knowledgeExchange = Math.floor(
      ecosystemOverview.totalRelationships * 0.3,
    ); // Estimate
    ...
    const maxPossibleConnections =
      ecosystemOverview.totalWorkflows * (ecosystemOverview.totalWorkflows - 1);
    ...
    const influenceScore =
      ecosystemOverview.totalWorkflows > 0
  ...
  getEnhancementEffectiveness(workflowId?: string): EnhancementEffectiveness {
    ...
    const totalImpact = allEnhancements.reduce(
    ...
    const byType: Record<
      EnhancementType,
      { averageImpact: number; count: number; successRate: number }
    > = {} as any;
    ...
    for (const type of this.getEnhancementTypes()) {
      ...
      const typeSuccessful = typeEnhancements.filter(
        (e) => (e.actualImpact || 0) > 0,
      );

      byType[type] = {
        averageImpact:
          typeEnhancements.length > 0
            ? typeEnhancements.reduce(
                (sum, e) => sum + (e.actualImpact || 0),
                0,
              ) / typeEnhancements.length
            : 0,
    ...
    for (const source of sources) {
      const sourceEnhancements = allEnhancements.filter(
        (e) => this.inferEnhancementSource(e) === source,
      );
      const sourceSuccessful = sourceEnhancements.filter(
        (e) => (e.actualImpact || 0) > 0,
      );

      bySource[source] = {
        averageImpact:
          sourceEnhancements.length > 0
            ? sourceEnhancements.reduce(
                (sum, e) => sum + (e.actualImpact || 0),
                0,
              ) / sourceEnhancements.length
            : 0,
    ...
    return {
      averageImpact:
        allEnhancements.length > 0 ? totalImpact / allEnhancements.length : 0,
      bySource,
      byType,
      successRate:
  ...
  getLearningEfficiency(workflowId?: string): LearningEfficiency {
    ...
    const recentEnhancements = workflowId
      ? (this.enhancementHistory.get(workflowId) || []).filter(
          (e) => new Date(e.validation.validatedAt).getTime() > windowStart,
        )
    ...
    const retainedKnowledge = recentEnhancements.filter(
      (e) => (e.actualImpact || 0) > 0.1 && e.validation.confidence > 0.7,
    ).length;
    ...
    const crossDomainEnhancements = recentEnhancements.filter(
      (e) =>
        this.inferEnhancementSource(e) === "peer" ||
  ...
  private calculateAdaptabilityScore(workflowId?: string): number {
    ...
    const successfulAdaptations = recentEnhancements.filter(
      (e) => (e.actualImpact || 0) > 0,
    );
    const successScore =
      recentEnhancements.length > 0
  ...
  private calculateWorkflowAnalytics(
    ...
    const enhancementEffectiveness =
      this.getEnhancementEffectiveness(workflowId);
  ...
  private inferEnhancementSource(enhancement: Enhancement): LearningSource {
    ...
    if (
      enhancement.description.includes("error") ||
      enhancement.type === "bug_fix"
    ) {
      return "error";
    }
    if (
      enhancement.description.includes("user") ||
      enhancement.type === "user_experience"
    ) {
      return "user";
    }
    if (
      enhancement.description.includes("peer") ||
      enhancement.description.includes("adapted")
    ) {
      return "peer";
    }
    if (
      enhancement.description.includes("analytics") ||
      enhancement.type === "optimization"
    ) {
      return "analytics";
    }
    if (
      enhancement.description.includes("composition") ||
