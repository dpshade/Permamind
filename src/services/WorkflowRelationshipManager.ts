...
export class WorkflowRelationshipManager {
  ...
  private hasComplementaryCapabilities(
    ...
    const hasRequirementCapabilityMatch = this.hasRequirementCapabilityMatch(
      links1,
      links2,
    );
    const hasComplementaryTypes = this.hasComplementaryRelationshipTypes(
      links1,
      links2,
    );
  ...
  private hasComplementaryRelationshipTypes(
    links1: MemoryLink[],
    links2: MemoryLink[],
  ): boolean {
    ...
    const complementaryPairs: Array<[RelationshipType, RelationshipType]> = [
      ["enhances", "depends_on"],
      ["supports", "references"],
      ["triggers", "causes"],
      ["extends", "inherits"],
    ...
    return complementaryPairs.some(
      ([type1, type2]) =>
        (types1.has(type1) && types2.has(type2)) ||
  ...
  private hasProducerConsumerPattern(
    links1: MemoryLink[],
    links2: MemoryLink[],
  ): boolean {
    ...
    const outputs1 = links1.filter((l) =>
      ["enhances", "supports", "triggers"].includes(l.type),
    );
    const inputs2 = links2.filter((l) =>
      ["depends_on", "references"].includes(l.type),
    );

    return outputs1.some((output) =>
      inputs2.some((input) => input.targetId === output.targetId),
    );
  }

  private hasRequirementCapabilityMatch(
    links1: MemoryLink[],
    links2: MemoryLink[],
  ): boolean {
    // Check if one workflow's requirements match the other's capabilities
    const requirements1 = links1.filter((l) => l.type === "depends_on");
    const capabilities2 = links2.filter((l) =>
      ["enhances", "supports"].includes(l.type),
    );

    return requirements1.some((req) =>
      capabilities2.some((cap) => cap.targetId === req.targetId),
    );
  }
