...
import { createTokenProcess, send, TokenDeploymentConfig } from "../process.js";
...
export class MarkdownWorkflowService {
  ...
  generateWorkflowSummary(markdownWorkflow: string): string {
    ...
    const exampleSection = markdownWorkflow.match(
      /\*\*Natural Language Examples:\*\*(.*?)(?=\*\*|---)/gs,
    );
    if (exampleSection) {
  ...
  private extractActionMapping(
    ...
    for (const section of actionSections) {
      ...
      const exampleSection = section.match(
        /\*\*Natural Language Examples:\*\*(.*?)(?=\*\*|$)/s,
      );
      if (exampleSection) {
  ...
  private extractKeywordsFromExamples(examples: string): string[] {
    ...
    for (const example of quotedExamples) {
      ...
      const actionWords =
        text.match(
          /(transfer|send|move|check|balance|mint|create|burn|destroy|approve|allow|info|details|allowance)/g,
        ) || [];
  ...
  private async extractMessageComponents(
    ...
    const actionSection = this.extractActionSection(
      markdownWorkflow,
      detectedAction,
    );
    if (actionSection) {
  ...
  private extractParametersFromRequest(
    ...
    const mappingSection = actionSection.match(
      /\*\*Parameter Mapping:\*\*(.*?)(?=\*\*|$)/s,
    );
    if (!mappingSection) {
    ...
    for (const pattern of amountPatterns) {
      ...
      if (match) {
        ...
        reasoning.push(
          `Extracted amount: ${amount} tokens = ${smallestUnits} smallest units`,
        );
        break;
      }
    }
    ...
    for (const pattern of supplyPatterns) {
      ...
      if (match) {
        ...
        if (
          userRequest.toLowerCase().includes("million") ||
        ...
        reasoning.push(
          `Extracted total supply: ${supply} = ${totalSupply} smallest units`,
        );
        break;
      }
    }
  ...
  private async handleTokenDeployment(
    ...
    try {
      ...
      reasoningChain.push(
        `Creating token with config: ${JSON.stringify(config)}`,
      );
      ...
      reasoningChain.push(
        `Token deployed successfully with process ID: ${processId}`,
      );
  ...
  private parseWorkflowMetadata(markdownWorkflow: string): {
    ...
    // Extract process ID
    const processIdMatch = markdownWorkflow.match(
      /\*\*Process ID:\*\*\s*`([^`]+)`/,
    );
    if (processIdMatch) {
