  ...
  validateConfig(client, configPath) {
    ...
    try {
      ...
      if (!config.mcpServers || !config.mcpServers.permamind) {
        return { valid: false, error: 'Permamind server configuration not found' };
      }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (args.length === 0) {
    ...
    console.log('');
    console.log('Usage:');
    console.log('  node configure.js list                    # List all configurations');
    console.log('  node configure.js claude <seed-phrase>   # Configure Claude Desktop');
    console.log('  node configure.js vscode <seed-phrase>   # Configure VS Code');
    console.log('  node configure.js cursor <seed-phrase>   # Configure Cursor');
    console.log('  node configure.js validate <client>      # Validate configuration');
    console.log('  node configure.js env <seed-phrase>      # Show environment setup');
    process.exit(0);
  }
  ...
  try {
    switch (command) {
      ...
      case 'validate':
        if (!args[1]) {
          console.error('Error: Client name is required (claude, vscode, cursor)');
          process.exit(1);
        }
        
        const client = args[1];
        const clientConfigPath = configurator.getConfigPath(client);
        const validation = configurator.validateConfig(client, clientConfigPath);
        
        if (validation.valid) {
          console.log(`✓ ${client.toUpperCase()} configuration is valid`);
        } else {
          console.log(`✗ ${client.toUpperCase()} configuration is invalid: ${validation.error}`);
          process.exit(1);
        }
        break;
      ...
      case 'env':
        ...
        const instructions = configurator.generateEnvInstructions(envSeedPhrase);
