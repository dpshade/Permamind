...
import { createProcess, read, send } from "./process.js";
...
function isReadOnlyIntent(intentType, tags) {
    ...
    execute: async (args) => {
        ...
        const executeWithTimeout = async () => {
            try {
                ...
                const processingResult = await nlProcessorService.processRequest(args.prompt, {
                    sessionId: args.sessionId,
                    userId: args.userId,
                });
                ...
                if (!keyPair) {
                    return JSON.stringify({
                        error: {
                            ...
                            message: "Wallet not initialized. Please wait for server startup or run setupPermamind.",
                            suggestions: [
                ...
                switch (intent.type) {
                    ...
                    case "custom_message":
                        ...
                        tags = messageFactory.CustomAction(intent.extractedParams.action, intent.extractedParams.customTags);
                    ...
                    case "query_state":
                        ...
                        break;
                    case "transfer":
                        if (!intent.extractedParams.recipient ||
                        ...
                        tags = messageFactory.Transfer(intent.extractedParams.recipient, intent.extractedParams.amount, intent.extractedParams.token || "");
                        break;
                    default:
                        return JSON.stringify({
                            error: {
                ...
                if (!targetProcessId) {
                    return JSON.stringify({
                        error: {
                            ...
                            message: "No target process specified and none could be created",
                            suggestions: [
                ...
                if (args.storeInMemory !== false) {
                    try {
                        const memoryContent = {
                            ...
                            importance: intent.confidence === "high"
                                ? 0.8
                ...
                return JSON.stringify({
                    ...
                    success: true,
                    suggestions: intent.confidence !== "high"
                        ? [
        ...
        return await Promise.race([
            executeWithTimeout(),
            new Promise((_, reject) => setTimeout(() => reject(new Error("aoMessage operation timed out after 60 seconds")), 60000)),
        ]);
    },
    name: "aoMessage",
    parameters: z.object({
        ...
        handlerDocs: z
            .string()
            .optional()
            .describe("Markdown documentation describing the process handler interface and message formats"),
        processId: z
            .string()
            .optional()
            .describe("Specific AO process ID to target (will be auto-determined if not provided)"),
        prompt: z
            .string()
            .describe("Natural language description of what you want to do with AO (e.g., 'send alice 10 AO', 'launch token called Flow', 'get my balance')"),
        sessionId: z
            .string()
            .optional()
        ...
        storeInMemory: z
            .boolean()
            .optional()
            .describe("Whether to store this interaction in memory for future reference (default: true)"),
        userId: z
            .string()
            .optional()
