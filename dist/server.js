...
server.addTool({
    ...
    execute: async (args) => {
        try {
            ...
            const response = await aoMessageService.executeMessage(workflowDefinition, request);
...
Reasoning: ${response.reasoningChain?.join(" → ") || "Not available"}`;
    ...
    parameters: z.object({
        markdownWorkflow: z
            .string()
            .describe("Markdown documentation describing the AO process, its actions, and parameter mapping. Should be human-readable workflow documentation."),
        processId: z
            .string()
            .optional()
        ...
        request: z
            .string()
            .describe("Natural language request describing what you want to do (e.g., 'transfer 100 tokens to alice', 'check my balance', 'mint 500 tokens')"),
        storeAsMemory: z
            .boolean()
            .optional()
            .describe("Whether to store this interaction as a memory (default: true)"),
...
server.addTool({
    ...
    execute: async (args) => {
        try {
            ...
            const memories = await aiMemoryService.searchAdvanced(hubId, searchQuery, {
                domain: "workflow_definitions",
            ...
            const workflowDefinitions = memories
                ...
                try {
                    ...
                    const jsonMatch = content.match(/Full Definition:\s*(\{[\s\S]*\})$/);
                    if (jsonMatch) {
