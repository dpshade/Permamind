 ...
 */
export const DEFAULT_TOKEN_PROCESS = {
 ...
 */
export function getDefaultTokenProcess(processId) {
    return {
 ...
 */
export function isTokenProcess(handlers) {
    const lowerHandlers = handlers.map(h => h.toLowerCase());
    // Check if it has core token handlers
    const hasCoreHandlers = TOKEN_DETECTION_PATTERNS.coreHandlers.every(core => lowerHandlers.includes(core));
    if (hasCoreHandlers)
        return true;
    // Check if it has enough token-like handlers
    const tokenHandlerCount = TOKEN_DETECTION_PATTERNS.handlers.filter(handler => lowerHandlers.includes(handler)).length;
