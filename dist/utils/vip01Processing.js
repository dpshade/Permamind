 ...
 */
export function applyClientSideFiltering(events, filter) {
    let filtered = [...events];
 ...
 */
export function extractTimestamp(event) {
    if (!event || typeof event !== "object") {
        return 0;
    }
 ...
 */
export function extractTimestamps(events) {
 ...
 */
export function generateFilterStats(filter, originalCount, finalCount) {
    const parameters = [];
    ...
    const efficiency = originalCount > 0 ? (originalCount - finalCount) / originalCount : 0;
    return {
 ...
 */
export function isProperlysorted(events) {
 ...
 */
export function processVIP01Results(events, filter, options = {}) {
    const { enableClientLimiting = true, enableSorting = true, includeMetadata = false, } = options;
