import { createTokenProcess, send } from "../process.js";
/**
...
export class MarkdownWorkflowService {
     ...
     */
    async executeWorkflowRequest(markdownWorkflow, userRequest, processId) {
        ...
        try {
            ...
            if (!targetProcessId ||
                targetProcessId === "YOUR_TOKEN_PROCESS_ID_HERE") {
                return {
                    error: {
            ...
            const messageComponents = await this.extractMessageComponents(markdownWorkflow, userRequest);
            ...
            // Send the message
            const rawResponse = await send(this.keyPair, targetProcessId, tags, messageComponents.data || null);
            // Format the response
            const formattedResponse = this.formatResponse(rawResponse, messageComponents.action, markdownWorkflow);
            return {
     ...
     */
    generateWorkflowSummary(markdownWorkflow) {
     ...
     */
    getAvailableActions(markdownWorkflow) {
        const actions = [];
        const sections = markdownWorkflow.split(/^### /m).slice(1);
        for (const section of sections) {
            const actionName = section.split("\n")[0].trim();
            if (actionName &&
                !actionName.includes("Configuration") &&
                !actionName.includes("Notes")) {
     ...
     */
    detectAction(userRequest, actionMapping) {
     ...
     */
    extractActionMapping(markdownWorkflow) {
        const mapping = {};
     ...
     */
    extractActionSection(markdownWorkflow, action) {
     ...
     */
    async extractMessageComponents(markdownWorkflow, userRequest) {
        ...
        if (actionSection) {
            const parameters = this.extractParametersFromRequest(userRequest, actionSection, markdownWorkflow);
     ...
     */
    extractParametersFromRequest(userRequest, actionSection, fullWorkflow) {
        const params = {};
        const reasoning = [];
        ...
        const fromPatterns = [/from\s+([a-zA-Z0-9]+)/i, /([a-zA-Z0-9]+)\s+to\s+/i];
        ...
        for (const pattern of supplyPatterns) {
            ...
            if (match) {
                ...
                if (userRequest.toLowerCase().includes("million") ||
                    userRequest.toLowerCase().includes("mil")) {
                    supply *= 1000000;
                }
        ...
        const decimalPatterns = [/(\d+)\s*decimals?/i];
     ...
     */
    formatResponse(rawResponse, action, markdownWorkflow) {
     ...
     */
    async handleTokenDeployment(messageComponents, startTime) {
        try {
            ...
            const config = {
                denomination: parameters.Denomination
                    ? parseInt(parameters.Denomination)
                    : 12,
     ...
     */
    parseWorkflowMetadata(markdownWorkflow) {
        const metadata = {};
        // Extract process ID
