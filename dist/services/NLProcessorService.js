...
const isValidProcessId = (processId) => /^[A-Za-z0-9_-]{43}$/.test(processId);
const isValidAmount = (amount) => /^\d+(?:\.\d+)?$/.test(amount) && parseFloat(amount) > 0;
const isValidTokenSymbol = (symbol) => /^[A-Z]{2,10}$/.test(symbol.toUpperCase());
class NLProcessorServiceImpl {
    async extractParameters(prompt, intentType) {
        const params = {};
    ...
    async getAlternativeIntents(prompt) {
        const alternatives = [];
            ...
            try {
                ...
                if (hasValidParams) {
                    alternatives.push({
                        ...
                        requiresProcessCreation: this.determineProcessCreationNeed(type, params),
                        type,
                    });
                }
            }
    ...
    async parseIntent(prompt, context) {
        const normalizedPrompt = prompt.trim().toLowerCase();
        // Try to match against known patterns
        for (const [type, patterns] of Object.entries(INTENT_PATTERNS)) {
            for (const pattern of patterns) {
                if (pattern.test(normalizedPrompt)) {
                    const confidence = this.calculateConfidence(normalizedPrompt, pattern);
                    const extractedParams = await this.extractParameters(prompt, type);
                    return {
                        confidence,
                        extractedParams,
                        originalPrompt: prompt,
                        processType: this.determineProcessType(type, extractedParams),
                        requiresProcessCreation: this.determineProcessCreationNeed(type, extractedParams),
                        type: type,
                    };
                }
            }
    ...
    requiresProcessCreation(intent) {
        return intent.requiresProcessCreation;
    }
    validateParameters(params, intentType) {
        const errors = [];
        switch (intentType) {
            case "create_token":
                if (!params.tokenName) {
            ...
            case "custom_message":
                if (!params.processId) {
                    ...
                    errors.push({
                        ...
                        suggestion: "Process ID should be 43 characters of letters, numbers, - or _",
                    });
                }
                if (!params.action) {
                    errors.push({
    ...
    determineProcessCreationNeed(type, params) {
        switch (type) {
            case "create_process":
            case "create_token":
                return true;
    ...
    determineProcessType(type, params) {
        switch (type) {
            case "create_process":
                if (params.processName) {
                    for (const [processType, pattern] of Object.entries(PROCESS_TYPE_PATTERNS)) {
    ...
    extractProcessName(prompt) {
        const match = prompt.match(/(?:create|launch|deploy|make)\s+(?:a\s+)?(\w+(?:\s+\w+)*)\s+(?:contract|process)/i);
        return match ? match[1] : undefined;
    }
    extractQueryType(prompt) {
        const patterns = [
    ...
    generateSuggestions(intent, errors) {
        const suggestions = [];
        if (intent.type === "unknown") {
            suggestions.push("Try rephrasing your request with clearer action words like 'send', 'create', 'launch', or 'get'", "Examples: 'send alice 10 AO', 'launch token called Flow', 'get my balance'");
        }
        if (errors.length > 0) {
        ...
        if (intent.confidence === "low") {
            suggestions.push("Your request was unclear. Consider being more specific about:");
            switch (intent.type) {
                case "create_token":
                ...
                case "custom_message":
                    ...
                    break;
                case "transfer":
                    suggestions.push("- Who to send to", "- How much to send", "- Which token");
                    break;
            }
        }
        return suggestions;
    }
    hasMinimumValidParams(params, type) {
        switch (type) {
            case "create_token":
...
export const nlProcessorService = new NLProcessorServiceImpl();
