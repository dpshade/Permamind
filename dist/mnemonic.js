import { generateMnemonic as bip39Generate, validateMnemonic as bip39Validate, mnemonicToSeed, wordlists, } from "bip39-web-crypto";
...
export async function getKeyFromMnemonic(mnemonic) {
    ...
    const jwk = pkcs8ToJwk(privateKey);
    return jwk;
}
/**
 ...
 *
 */
export async function pkcs8ToJwk(privateKey) {
    const key = await crypto.subtle.importKey("pkcs8", privateKey, { hash: "SHA-256", name: "RSA-PSS" }, true, ["sign"]);
