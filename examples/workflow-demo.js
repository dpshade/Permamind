...
console.log(`  Current performance: ${stats.current?.qualityScore?.toFixed(2)} quality score`);
console.log(`  Average performance: ${stats.average?.qualityScore?.toFixed(2)} quality score`);
...
relationshipManager.createRelationship('data-processor-v2', 'data-processor', 'enhances', 0.9);
relationshipManager.createRelationship('data-analyzer', 'data-processor', 'depends_on', 0.8);
relationshipManager.createRelationship('report-generator', 'data-analyzer', 'depends_on', 0.7);

const relationships = relationshipManager.getRelationships('data-processor');
console.log(`  data-processor has ${relationships.length} outgoing relationships`);

const dependents = relationshipManager.getDependentWorkflows('data-processor');
console.log(`  ${dependents.length} workflows depend on data-processor: ${dependents.join(', ')}`);

const networkMetrics = relationshipManager.calculateNetworkMetrics('data-processor');
console.log(`  Network connectivity: ${(networkMetrics.connectivityScore * 100).toFixed(1)}%`);
console.log(`  Influence score: ${(networkMetrics.influenceScore * 100).toFixed(1)}%`);
...
const enhancements = performanceTracker.identifyEnhancements('declining-workflow');
...
enhancements.forEach((enhancement, index) => {
  ...
  console.log(`       Expected impact: ${(enhancement.impact * 100).toFixed(1)}%`);
});
...
console.log(`  Total workflow types: ${Object.keys(analytics.workflowDistribution).length}`);
...
console.log(`  Average connectivity: ${ecosystemOverview.averageConnectivity.toFixed(2)}`);
console.log(`  Hub workflows: ${ecosystemOverview.hubWorkflows.length}`);
console.log(`  Isolated workflows: ${ecosystemOverview.isolatedWorkflows.length}`);
...
enhancementEngine.initializeEnhancementLoop('self-improving-workflow', optimizationTargets);
...
console.log(`  User feedback generated ${userEnhancements.length} enhancement suggestions:`);
